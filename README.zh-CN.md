...
- **多任务工作区**：可并行进行多个互不相关的生图任务，每个任务独立使用各自的 API、提示词、参数与参考图。
- **粘贴参考图**：在任意位置粘贴复制好的图片或图片数据（data URL）即可加入参考图，无需本地文件路径。
- **模型名映射**：将中转站模型名映射到内置 Gemini/GPT 模型；拆分模式按 1K/2K/4K 匹配，缺失档位在请求前拦截。
- **可配置队列并发**：支持 1-8 个队列任务并行执行。
- **Gemini 像素尺寸适配**：按供应商开关，改用 WxH 像素（如 9:16 2K 对应 1152x2048）发送，让忽略 aspectRatio 字段的中转站也能按所选比例出图。
- **任务复制/粘贴**：右键任务页签可复制整个任务（提示词、参数、参考图以 base64 保存）或仅复制提示词，并粘贴到其它页签。
- **暗色模式**：适合长时间选图、对图和编辑。

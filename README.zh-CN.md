...
- 多个 API 接入在左侧栏统一管理，界面文案统一为 API 接入。
- 当前正在使用的 API 接入以紧凑信息卡展示，包含 API 类型、Base URL、Key 保存状态和探测模型数。
- 已保存但未使用的 API 接入默认折叠，用户可以随时展开、切换或编辑。
- 提示词模板提供左侧栏入口和独立管理器，支持新建、编辑、搜索、标签筛选、导入、导出和应用。
...
| API 接入配置 | 当前 API 接入以紧凑信息卡展示，需要编辑 API Key、Base URL、API 类型或探测信息时再展开。 |
...
| 提示词模板 | 左侧模板区与独立管理器支持保存、搜索、打标签、导入、导出和应用常用提示词。 |
...
| 平台 | 文件 | 状态 |
| --- | --- | --- |
| macOS Apple Silicon | `Image2Tools-0.2.4-mac-arm64.dmg` | 本机签名身份可用时使用 Developer ID 签名。Apple 公证取决于 notary 凭据是否配置。 |

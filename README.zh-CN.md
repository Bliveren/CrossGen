...
CrossGen 0.3.1 在一站式 AI 生图管理工作台的基础上，增加了面向本地 coding agent 的 CLI/MCP 运行时。本地 agent 可以发现 provider 和模型能力、提交排队生图任务、轮询任务状态，并把图库资产导出到项目目录。
...
CrossGen 0.3.1 通过 JSON CLI 和 MCP stdio server 暴露本地运行时。桌面端、CLI 和 MCP 使用同一套队列与图库规则：

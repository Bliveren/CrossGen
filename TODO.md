...
- [x] 实现参考图选择上传
- [x] 实现多图参考输入
- [x] 将编辑输入/参考图数量限制为 GPT Image 2 支持的最多 16 张
- [x] 实现编辑模式切换
- [x] 实现局部重绘入口
- [x] 实现 mask 画布或遮罩上传
...
- [ ] 完成签名、公证并补充正式分发资产 URL / hash / size 证据
  - 2026-06-16: 该项降级为非阻塞增强；签名与公证证据已拆分为 `macos-signed` 与 `macos-notarized`。
  - 2026-06-30: `v0.2.4` 已完成 Developer ID signed 资产并记录为 `macos-signed` passed；Apple notarization 因凭据未配置仍为 `macos-notarized` blocked / required=false。取得 Apple notary credentials 后再将 `macos-notarized` 更新为 passed，并决定是否改回 required=true。

...
const bridge: AppBridge = {
  ...
  installCrossgenArtistSkill: () => ipcRenderer.invoke("app:installCrossgenArtistSkill") as Promise<AgentRuntimeStatus>,
  ...
  addHistoryAssetToGallery: (assetPath: string, folderId?: string | null, tags?: string[]) => ipcRenderer.invoke("gallery:addHistoryAsset", assetPath, folderId, tags),
  addEditedImageToGallery: (input: EditedGalleryImageInput) => ipcRenderer.invoke("gallery:addEditedImage", input),
  replaceGalleryAssetImage: (id: string, input: EditedGalleryImageInput) => ipcRenderer.invoke("gallery:replaceImage", id, input),
  ...
  updateHistoryJob: (jobId: string, patch: HistoryJobPatch) => ipcRenderer.invoke("history:updateJob", jobId, patch),

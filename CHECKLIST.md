...
- [x] 产品负责人已完成本地安装包实测并明确批准发布
  - 硬性规则：没有产品负责人对“同一候选提交、同一安装包”的明确实测批准，不允许发布 GitHub Release，不允许标记 Latest，不允许更新 `docs/updates/latest.json` 指向该版本。
  - 自动测试、签名、公证、CI、mock/真实 API 验收只能作为必要条件，不能替代产品实测。

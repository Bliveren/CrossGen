...
describe("macOS release verifier dmg selection", () => {
  it("selects the package-version dmg when older release artifacts remain", () => {
    expect(selectDmgFile([
      file("CrossGen-0.2.2-mac-arm64.dmg"),
      file("CrossGen-0.2.3-mac-arm64.dmg"),
      file("CrossGen-0.2.3-mac-arm64.zip"),
      directory("mac-arm64")
    ], "0.2.3")).toBe(path.resolve("release/CrossGen-0.2.3-mac-arm64.dmg"));
  });

  it("still fails when multiple dmgs exist for the current package version", () => {
    expect(() => selectDmgFile([
      file("CrossGen-0.3.0-mac-arm64.dmg"),
      file("CrossGen-0.3.0-mac-x64.dmg")
    ], "0.3.0")).toThrow("Expected one CrossGen macOS dmg for version 0.3.0");
  });

  it("falls back to the old single-candidate behavior when no versioned dmg exists", () => {
    expect(selectDmgFile([
      file("CrossGen-preview-mac-arm64.dmg")
    ], "0.3.0")).toBe(path.resolve("release/CrossGen-preview-mac-arm64.dmg"));
  });
});

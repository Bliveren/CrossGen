...
| Cross-platform install validation | Windows and Linux package gates are configured in GitHub Actions; `pnpm verify:release:linux` automates Linux AppImage/unpacked executable inspection, unpacked app Xvfb launch, AppImage extraction, and extracted app Xvfb launch. Ubuntu 24.04 ARM64 Docker validation on the macOS host passed build, mock API verification, Linux AppImage packaging, AppImage extraction, and extracted-app launch under Xvfb. Direct AppImage execution is still blocked in Docker by missing FUSE device support, and native Windows plus native Linux desktop shell checks are still pending; tracked in GitHub issue #4 | Partially verified; external/manual pending |

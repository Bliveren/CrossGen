# v0.3.3 Release Preflight (Archived)

> The v0.3.3 release is now public. This preflight is the historical candidate
> checklist; final release evidence is recorded in `docs/release/evidence.json`.
...
- [x] 完成 Agent Access runtime contract 定向测试与 `pnpm typecheck`。
- [x] 完成受管 POSIX link / Windows shim 的创建、拒绝覆盖和清理测试。
- [x] 从精确 `v0.3.3` 候选源码生成并验证至少一个可安装或可运行的候选包。
- [x] 在精确候选上运行 build、CLI/MCP smoke、agent integration smoke 和图片核心回归。
- [x] 在原生 Windows/Linux runner 上完成精确候选包验证。
- [x] 完成精确候选的真实 provider gate。
- [x] 完成产品负责人安装实测并记录明确批准。
- [x] 所有必要 release evidence gate 通过后，才允许 tag、Latest 和 manifest promotion。

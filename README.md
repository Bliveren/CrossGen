...
真实验收还要求对应 OpenAI 组织已完成 GPT Image 所需的组织验证。开启流式 `partial_images` 会产生额外输出 token 成本；如果还要让脚本额外执行真实 streaming generation 检查，需要在上述成本确认之外再设置 `IMAGE2TOOLS_REAL_API_ACCEPT_STREAM_COST=1`。
